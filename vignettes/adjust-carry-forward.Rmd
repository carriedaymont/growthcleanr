---
title: "Testing carried forward adjustments"
date: "`r Sys.Date()`"
vignette: >
  %\VignetteIndexEntry{Testing carried forward adjustments}
...
The primary tool from running this function is the script `exec/testadjustcf.R`.
This will run the experimental function on an existing dataset (specified as a
CSV file) and pass it a sweep of parameter values for testing the new strategy.
This tool will produce a combined output file that includes the original result
...
First navigate to the growthcleanr package directory in the command line. Then
...
option, and can be changed `--exclude_opt`. More information on each option can
...
given dataset have been incorporated. When the `--add_answers` flag is set to
...
script; for runs 1 and 2, the result is not changes, but for runs 3-5, the
