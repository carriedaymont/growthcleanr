...
- For cleaning pediatric data, ensure that `growthcleanr` uses a consistent set
  ...
  derived from NHANES. The child algorithm always uses the built-in reference
  file (`rcfile-2023-08-15_format.csv.gz`) for recentering, so this is handled
  automatically and recentering will be consistent across all runs. Custom
  recentering is supported via the `sd.recenter` parameter if needed.
...
- The child algorithm uses a built-in reference file for recentering, so
  recentering is automatically consistent across all runs.
