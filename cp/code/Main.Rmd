...
# data.work <- data.work  %>% filter(!subjid %in% rem_ids$subjid.x)
data.work <- data.work %>% filter(subjid %in% ewmas_of_int$subjid.x)
data.work <- as.data.table(data.work)
setkey(data.work, subjid, param, agedays)
cleaned_data_updated <- cleangrowth(
  subjid = data.work$subjid,
...
### EWMA Spot Checks.
# Read in the stata.
stata_ewma_facts_in <- read.csv("../data/temp-2025-10-24.csv") %>% filter(subjid %in% ewmas_of_int$subjid.x)

stata.gc <- stata_ewma_facts_in  %>% 
  mutate(param = case_when(
  param == "Weight" ~ "WEIGHTKG",
  param == "Height" ~ "HEIGHTCM",
  TRUE ~ param
...
stata.gc <- as.data.table(stata.gc)
setkey(stata.gc, subjid, param, agedays)
stata_data_updated <- cleangrowth(
  subjid = stata.gc$subjid,
  param = stata.gc$param,
  agedays = stata.gc$agedays,
  sex = stata.gc$sex,
  measurement = stata.gc$measurement,
  prelim_infants = TRUE,
  id = stata.gc$obsid, 
  sd.recenter = "nhanes"
)
stata_data_updated <- as.data.frame(stata_data_updated)
stata_data_updated$subjid <- as.numeric(as.character(stata_data_updated$subjid))

ewma_lead <- read.csv("../data/sde_debug_NNNNN.csv")

data.work_2 <- merge(stata.gc %>% select(subjid, obsid), ewma_lead, by.x = c("subjid", "obsid"), by.y = c("subjid", "line"))

stata.gc$obsid
# ### Isolate some examples of mismatches in a ewma step - this is where ewma fires for stata but not for R.
# ewmas_of_int <- Merged_Maps %>% group_by(subjid.x) %>% filter((gcr_result_carrie == "EWMA2 first ext"  & gcr_result_cp == "Include" & gcr_result_cp_updated == "Include"))

# Filter the run to those folks only.
# Done above.

# Isolate. Read in the lead.
...
datsel <- stata_in %>% select(subjid, param, agedays_stata = agedays, id = obsid, measurement, exc_ht, exc_wt,ewma_wt, ewma_ht,  absdewma_wt, absdewma_ht,  minabsdewma_wt, minabsdewma_ht, maxagediff_wt, maxagediff_ht)%>%
  mutate(param = case_when(
  param == "Weight" ~ "WEIGHTKG",
  param == "Height" ~ "HEIGHTCM",
  TRUE ~ param
)) %>% filter(param %in% c("WEIGHTKG", "HEIGHTCM")) %>% mutate(
...
ewma = case_when(
  param == "WEIGHTKG" ~ ewma_wt,
  param == "HEIGHTCM" ~ ewma_ht
),
absdewma = case_when(
  param == "WEIGHTKG" ~ absdewma_wt,
  param == "HEIGHTCM" ~ absdewma_ht
),
minabsdewma = case_when(
  param == "WEIGHTKG" ~ minabsdewma_wt,
  param == "HEIGHTCM" ~ minabsdewma_ht
),
maxagediff = case_when(
  param == "WEIGHTKG" ~ maxagediff_wt,
  param == "HEIGHTCM" ~ maxagediff_ht
)
) %>% select(-c(exc_ht, exc_wt,ewma_wt, ewma_ht,  absdewma_wt, absdewma_ht,  minabsdewma_wt, minabsdewma_ht, maxagediff_wt, maxagediff_ht))

datsel2 <- read.csv("../data/sde_debug_NNNNN.csv") 
datsel2 <- datsel2# %>% select(subjid, id, ewma.all, ewma.before, ewma.after)
datsel3 <- merge(datsel, datsel2, by = "id")
datsel3
```
...
View(cleaned_data_updated)
 ...
 ewmas_of_int <- Merged_Maps %>% group_by(subjid.x) %>% filter((gcr_result_carrie == "EWMA2 first ext"  & gcr_result_cp == "Include" & gcr_result_cp_updated == "Include")) 
 ewmas_of_int2 <- Merged_Maps %>% group_by(subjid.x) %>% filter((gcr_result_carrie == "Include"  & gcr_result_cp == "EWMA2 first" & gcr_result_cp_updated == "EWMA2 first"))
 write.csv(bind_rows(ewmas_of_int, ewmas_of_int2) %>% select(subjid.x, id ), "../data/ewma_first_ext_and_first_issues.csv")
...
```{r}
# We want to focus on the next most problematic item, which is SDE-EWMA -> SDE-One-Day
sde_one_days <- Merged_Maps %>% group_by(subjid.x) %>% filter((gcr_result_carrie == "SDE-EWMA"  & gcr_result_cp == "SDE-One-Day" & gcr_result_cp_updated == "SDE-One-Day")) 

sde_ewma <- Merged_Maps %>% group_by(subjid.x) %>% filter((gcr_result_carrie == "Include"  & gcr_result_cp == "SDE-One-Day" & gcr_result_cp_updated == "SDE-One-Day")) 
sde_ewma2 <- Merged_Maps %>% group_by(subjid.x) %>% filter((gcr_result_carrie == "SDE-EWMA"  & gcr_result_cp == "Include" & gcr_result_cp_updated == "Include")) 
sde_ewma3 <- Merged_Maps %>% group_by(subjid.x) %>% filter((any(grepl("SDE",gcr_result_carrie) | grepl("SDE",gcr_result_cp)  | grepl("SDE",gcr_result_cp_updated))))

rem_ids <- bind_rows(sde_one_days, sde_ewma, sde_ewma2, sde_ewma3) %>% select(subjid.x) %>% distinct()
sde_one_days <- sde_one_days %>% select(subjid.x, id) %>% distinct() %>% select(subjid = subjid.x, 
                                                                                id)
rem_ids

# write.csv(sde_one_days, "../data/sde_ewma_nofires.csv")
```

```{r}
# We want to focus on the next most problematic item, which is SDE-EWMA -> SDE-One-Day
sde_one_days <- Merged_Maps %>% group_by(subjid.x) %>% filter(any(gcr_result_carrie == "Min diff" &gcr_result_cp == "Min diff" & gcr_result_cp_updated == "Include")) 
sde_one_days <- sde_one_days %>% select(subjid.x, id) %>% distinct()

View(cleaned_data_updated %>% filter(subjid == "76234"))
View(clean)
```
