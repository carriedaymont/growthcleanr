...
data.work <- data.work  %>% filter(subjid %in% sde_one_days$subjid.x)
data.work <- as.data.table(data.work)
setkey(data.work, subjid, param, agedays)
cleaned_data_updated <- cleangrowth(
  subjid = data.work$subjid,
...
sde_one_days <- Merged_Maps %>% group_by(subjid.x) %>% filter(any(gcr_result_carrie == "SDE-EWMA" &gcr_result_cp == "SDE-One-Day" & gcr_result_cp_updated == "SDE-One-Day")) 
sde_one_days <- sde_one_days %>% select(subjid.x, id) %>% distinct()

View(cleaned_data_updated %>% filter(subjid == "8008"))
View(clean)
```
