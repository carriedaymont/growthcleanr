...
data.work <- data.work # %>% filter(subjid %in% conflicted_pats)
data.work <- as.data.table(data.work)
setkey(data.work, subjid, param, agedays)
cleaned_data_updated <- cleangrowth(
  subjid = data.work$subjid,
...
cleaned_data_updated <- read.csv("../data/Checkpoint_Updated_R_2025-10-14_evening.csv")
...
Merged_Maps_sub <- Merged_Maps %>% filter(gcr_result_carrie == "Min diff" & gcr_result_cp == "Min diff" &	gcr_result_cp_updated == "Include")
# Merged_Maps_sub
moddf <- merge(data.work, cleaned_data_updated %>% select(-subjid), by = "id") %>% filter()

# View(moddf %>% filter(subjid %in% c(Merged_Maps_sub$subjid.x)) %>% select(subjid, agedays.x, param.x, sd.orig_uncorr, swtz, shtz,  measurement, v, gcr_result_carrie, exclude))
...
datsel <- merge(Baseline_Map, Updated_Map, by = c("id", "subjid"), all = TRUE) %>% group_by(subjid) %>% filter(any(gcr_result_carrie == "1 CF deltaZ <0.05" & gcr_result_cp_updated == "Exclude-Carried-Forward")) #%>% distinct(subjid)
Modified_Characteristics_Table <- merge(data.work, cleaned_data_updated %>% select(-subjid), by = "id")

View(Modified_Characteristics_Table %>% filter(subjid %in% c(datsel$subjid)) %>% select(subjid, agedays.x, param.x, sd.orig_uncorr, swtz, measurement, v, gcr_result_carrie, exclude))
...
```{R}
read.csv("../data/ewma2_debug_NNNNN.csv")
```
