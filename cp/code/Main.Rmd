...
stata_gc_in <- data.table::fread("../data/2025-11-09-gc-Stata-chopd1p-results.csv")
stata_gc_in_sde_pats <- stata_gc_in %>% group_by(subjid) %>% filter(any(grepl("SDE", exc_ht) | grepl("SDE", exc_wt))) %>% select(subjid) %>% distinct() 
data.work <- stata_gc_in %>% 
  select(
  id = obsid,
...
gt(tbl) %>%
  ...
  data_color(
    columns = c(gcr_result_cp_updated),
    rows = gcr_result_cp_updated != gcr_result_carrie,
    fn = function(x) rep("#F2994A", len  gth(x))
  ) %>%
  data_color(
    columns = c(gcr_result_cp_updated),
    rows = gcr_result_cp == gcr_result_carrie & gcr_result_cp_updated != gcr_result_cp,
...
IDs_of_interest <- Merged_Maps %>% filter(gcr_result_carrie == "SDE-EWMA" &
                         gcr_result_cp == "SDE-One-Day" & 
                         gcr_result_cp_updated == "SDE-One-Day") 

View(cleaned_data_updated %>% filter(subjid %in% IDs_of_interest$subjid.x))

# View(cleaned_data_updated %>% filter(subjid %in% IDs_of_interest$subjid.x) %>% mutate(interest_flag = ifelse(id %in% IDs_of_interest$id, 1, 0)))

# write.csv(IDs_of_interest, "../data/sde-oneday-folks-11-12-2025.csv")
