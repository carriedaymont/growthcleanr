...
# ------------------------------------------------------------
  ...
  fwrite(df, "../data/mindiff_debug_NNNNN.csv")
           }

      # calculate ewma
      df[, (ewma.fields) := ewma(agedays, tbc.sd, exp_vals, TRUE)]
      if (unique(df$subjid) == "444336" & unique(df$param == "HEIGHTCM")) {
  fwrite(df, "../data/mindiff_debug_NNNNN_lead_2.csv")}
      # calculate dewma
      df[, `:=`(
        dewma.all = tbc.sd - ewma.all,
        dewma.before = tbc.sd - ewma.before,
      ...
      if (unique(df$subjid) == "444336" & unique(df$param == "HEIGHTCM")) {
  fwrite(df, "../data/mindiff_debug_NNNNN_lead_3.csv")}
      ...
      if (nrow(df) > 2){
        ...
        df[pair & abs(dewma.before) > dplyr::lead(abs(dewma.after)),
           bef.g.aftm1 := TRUE]
        df[pair & abs(dewma.after) > dplyr::lag(abs(dewma.before)),
           aft.g.aftm1 := TRUE]
      ...
      if (unique(df$subjid) == "444336" & unique(df$param == "HEIGHTCM")) {
  fwrite(df, "../data/mindiff_debug_NNNNN_lead_4.csv")}
        else { # only 2 values
      ...
      # df[is.na(diff_next), diff_next := 0]
...
idx <- df$index[which.max(df[df$val_excl != "Include", ][["absval"]])]
        ...
        if ("index" %in% names(df) && any(df$index == 822)) {
  df_822 <- df[index == 822]
  df_822[, loop_iter := if (exists("loop_iter", envir = .GlobalEnv)) {
    get("loop_iter", envir = .GlobalEnv) + 1
  } else { 1 }]
  assign("loop_iter", df_822$loop_iter[1], envir = .GlobalEnv)

  fwrite(df_822,
         file = "../data/debug_index822_trace.csv",
         append = TRUE)
}
    ...
    return(exclude_all)
 # if (unique(exclude_all$subjid) == "444336") {
  # fwrite(exclude_all, "../data/exclude_all_444336.csv")}
