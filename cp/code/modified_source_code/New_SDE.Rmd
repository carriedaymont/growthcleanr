...
View(data.df %>% group_by(agedays, param, subjid) %>% filter(any(exclude == "Exclude-Temporary-Extraneous-Same-Day")))
```
  ...
  subset(valid(data.df, include.temporary.extraneous = TRUE) & 
           nnte_full == FALSE) %>% select(subjid, obsid = id, agedays, param, v, tbc.sd, exclude, nnte_full) %>% group_by(subjid, param, agedays) %>% mutate(
             sde_this = case_when(
               ...
               TRUE ~ FALSE)) %>% arrange(subjid, param, agedays, obsid) %>% group_by(subjid, param, agedays) %>% mutate(exclude = case_when(
                 length(unique(v)) == 1 & 
                 exclude != "Include" ~ "Exclude-Same-Day-Identical",
                        TRUE ~ exclude),
                  # group size of identical values
                  dup_count = ave(v, v, FUN = length),
                  # mark duplicates other than the one with the highest obsid
                  exclude = case_when(
                    dup_count > 1 & obsid != max(obsid[v == v], na.rm = TRUE) ~ "Exclude-SDE-Identical",
                    TRUE ~ exclude
                  ),
                  absdiff_rel_to_median = case_when(exclude %in% c("Include", "Exclude-Temporary-Extraneous-Same-Day") ~
 abs(tbc.sd-median(tbc.sd, na.rm = TRUE)),
 TRUE ~ NA)),
