...
#' @param param Vector identifying each measurement, may be 'WEIGHTKG', 'HEIGHTCM', 'LENGTHCM', or 'HEADCM'
...
cleangrowth = function(subjid,
  ...
  data.all=data.table(line=seq_along(measurement), subjid=as.factor(subjid), param, agedays=as.integer(agedays), v=ifelse(measurement==0,NaN,measurement), sex=as.integer(ifelse(sex %in% c(0,'m','M'), 0, ifelse(sex %in% c(1,'f','F'), 1, NA))))
  ...
  batches.all = data.table(subjid=subjid.unique, batch=sample(num.batches, length(subjid.unique), replace=T), key='subjid')
  ...
  tanner_ht_vel_path<-ifelse(ref.data.path=="",
  ...
  who_max_ht_vel_path<-ifelse(ref.data.path=="",
  ...
  exclude.levels = c('Include', 'Unit-Error-High', 'Unit-Error-Low', 'Unit-Error-Possible', 'Swapped-Measurements', 'Exclude', 'Missing',
  ...
  cleanbatch = function(data.df, log.path) {
    ...
    swap.parameters = function(param.1='WEIGHTKG', param.2='HEIGHTCM', field.name='tbc.sd', df=data.df) {
      valid.rows = valid(df)
      ...
      valid.other=df[valid.rows,list(subjid.other=subjid,
                                     param.other=ifelse(param==param.1, param.2, ifelse(param==param.2, param.1, NA)),
                                     agedays.other=agedays,swap)]
    ...
    if(!quietly) cat(sprintf("[%s] Identify potentially swapped measurements...\n", Sys.time()))
    ...
    data.df[valid(exclude), (ewma.fields) := ewma(agedays, tbc.sd, ewma.exp, T), by=.(subjid,param)]
    ...
    data.df[, `:=`(valid.interior.measurement=F, swap.flag.1=F)]

    # flag interior measurements
    data.df[valid(), valid.interior.measurement := if(.N>2) c(F, rep(T, .N-2), F) else F, by=.(subjid,param)]
    data.df[valid.interior.measurement &
              (param == 'WEIGHTKG' & tbc.sd > 4 & abs(tbc.sd.sw) < 3 & (tbc.sd - ewma.all) > 3 & (tbc.sd - ewma.before) > 2 & (tbc.sd - ewma.after) > 2
               & abs(tbc.sd.sw - ewma.all) < 0.3 & abs(tbc.sd.sw - ewma.before) < 0.5 & abs(tbc.sd.sw - ewma.after) < 0.5
               | param == 'HEIGHTCM' & tbc.sd < -7 & abs(tbc.sd.sw) < 3 & (tbc.sd - ewma.all) < -6 & (tbc.sd - ewma.before) < -5 & (tbc.sd - ewma.after) < -5
               & abs(tbc.sd.sw - ewma.all) < 0.3 & abs(tbc.sd.sw - ewma.before) < 0.5 & abs(tbc.sd.sw - ewma.after) < 0.5), swap.flag.1 := T]
    ...
    data.df[swap.flag.1 & swap.flag.2, `:=`(v=v.sw, tbc.sd=tbc.sd.sw, exclude='Swapped-Measurements')]
    ...
    if(recover.unit.error) {
      ...
      #    iii.	ht_d_2=ht/2.54
      #    iv.	ht_t_2=ht*2.54
      if(!quietly) cat(sprintf("[%s] Identify and recover unit errors...\n", Sys.time()))
      valid.rows = valid()
      data.df[, `:=`(v.d = v/ifelse(param == 'WEIGHTKG', 2.204622, 2.54),
      ...
      data.df$tbc.sd.d = with(data.df, measurement.to.z(param, agedays, sex, v.d, T) - sd.median)
      data.df$tbc.sd.t = with(data.df, measurement.to.z(param, agedays, sex, v.t, T) - sd.median)
      ...
      data.df[valid.rows, `:=`(delta.prev.sd=tbc.sd - c(NA, tbc.sd[-.N]),
                               delta.next.sd=tbc.sd - c(tbc.sd[-1], NA)), by=.(subjid,param)]
      ...
      data.df[valid() & abs(tbc.sd.d - ewma.all) < 0.3 & abs(tbc.sd.d - ewma.before) < 0.5 & abs(tbc.sd.d - ewma.after) < 0.5 & abs(tbc.sd.d) < 3
              & (param=='WEIGHTKG' & (tbc.sd - ewma.all) > 3 & (tbc.sd - ewma.before) > 2 & (tbc.sd - ewma.after) > 2 & tbc.sd > 3
                 & !is.na(delta.next.sd) & delta.next.sd > 2 & !is.na(delta.prev.sd) & delta.prev.sd > 2
                 | param=='HEIGHTCM' & (tbc.sd - ewma.all) > 5 & (tbc.sd - ewma.before) > 4 & (tbc.sd - ewma.after) > 4 & tbc.sd > 7
                 & !is.na(delta.next.sd) & delta.next.sd > 4 & !is.na(delta.prev.sd) & delta.prev.sd > 4 ),
              `:=`(v=v.d, tbc.sd=tbc.sd.d, exclude='Unit-Error-High')]

      # process unit error high second (8.e.ii and iv from above), re-factor slightly for efficiency
      data.df[valid() & abs(tbc.sd.t - ewma.all) < 0.3 & abs(tbc.sd.t - ewma.before) < 0.5 & abs(tbc.sd.t - ewma.after) < 0.5 & abs(tbc.sd.t) < 3
              & (param=='WEIGHTKG' & (tbc.sd - ewma.all) < -3 & (tbc.sd - ewma.before) < -2 & (tbc.sd - ewma.after) < -2 & tbc.sd < -3
                 & !is.na(delta.next.sd) & delta.next.sd < -2 & !is.na(delta.prev.sd) & delta.prev.sd < -2
                 | param=='HEIGHTCM' & (tbc.sd - ewma.all) < -5 & (tbc.sd - ewma.before) < -4 & (tbc.sd - ewma.after) < -4 & tbc.sd < -7
                 & !is.na(delta.next.sd) & delta.next.sd < -4 & !is.na(delta.prev.sd) & delta.prev.sd < -4 ),
              `:=`(v=v.t, tbc.sd=tbc.sd.t, exclude='Unit-Error-Low')]
    }
    ...
    if(!include.carryforward) {
      if(!quietly) cat(sprintf("[%s] Exclude measurements carried forward...\n", Sys.time()))
      ...
      data.df[!(subjid %in% subj.dup) & v.orig == prev.v, exclude := 'Exclude-Carried-Forward']

      # need to handle children with duplicate measurements on same day separately
      data.df[subjid %in% subj.dup & valid(include.temporary.duplicates=T), exclude := (function(df) {
        setkey(df,agedays)
        ages = unique(agedays)
        ...
        if (length(ages) > 1) {
          ...
          for(i in 2:length(ages)) {
            ...
            df[agedays == ages[i] & v.orig %in% all.prev.v, exclude := 'Exclude-Carried-Forward']
          }
        }
        return(df$exclude)
    ...
    data.df[J(dup.ratio.df[dup.ratio > 1/2,list(subjid,param)]), exclude := (function(df) {
      ...
      df[valid(exclude, include.duplicates=T, include.temporary.duplicates=T), `:=`(tbc.sd.min=min(tbc.sd), tbc.sd.max=max(tbc.sd))]
    ...
    data.df[subjid %in% subj.all.dups, exclude := (function(subj.df) {
      ...
      valid.rows = valid(subj.df, include.duplicates=T, include.temporary.duplicates=T)
      ...
      subj.df[duplicates.this.day & exclude %in% c('Exclude-SD-Cutoff', 'Exclude-EWMA-Extreme', 'Exclude-EWMA-Extreme-Pair'), exclude := 'Exclude-Duplicate']

      return(subj.df$exclude)
    })(copy(.SD)), .SDcols=c('param', 'agedays','exclude','tbc.sd'), by=.(subjid)]
    ...
    data.df[subjid %in% subj.dup, exclude := (function(subj.df) {
      if(.N > 1) {
        subj.df[exclude %in% c('Exclude-SD-Cutoff', 'Exclude-EWMA-Extreme', 'Exclude-EWMA-Extreme-Pair'), exclude := 'Exclude-Duplicate']
      }
      return(subj.df$exclude)
    ...
    data.df[, `:=`(tbc.sd.minus=measurement.to.z(param, agedays, sex, v.minus, T),
    ...
    data.df[param == 'HEIGHTCM', exclude := (function(subj.df) {
      ...
      while(T) {
        ...
        subj.df[valid(exclude), exclude := (function (df) {
          ...
          df[, `:=`(maxdiff.next.ht=as.double(NA), mindiff.next.ht=as.double(NaN))]
          ...
          #   153-198	      6
          #   All others	  missing
          df[, whoinc.age.ht := ifelse(delta.agedays.next < 20 , NA,
                                       ifelse(delta.agedays.next <= 45, 1,
                                              ifelse(delta.agedays.next <= 75, 2,
                                                     ifelse(delta.agedays.next <= 106, 3,
                                                            ifelse(delta.agedays.next <= 152, 4,
          ...
          df[, who.mindiff.next.ht := ifelse(delta.agedays.next < 20 , NA,
                                             ifelse(delta.agedays.next <= 45, whoinc.1.ht,
                                                    ifelse(delta.agedays.next <= 75, whoinc.2.ht,
                                                           ifelse(delta.agedays.next <= 106, whoinc.3.ht,
                                                                  ifelse(delta.agedays.next <= 152, whoinc.4.ht,
          ...
          df[, who.maxdiff.next.ht := ifelse(delta.agedays.next < 20 , NA,
                                             ifelse(delta.agedays.next <= 45, max.whoinc.1.ht,
                                                    ifelse(delta.agedays.next <= 75, max.whoinc.2.ht,
                                                           ifelse(delta.agedays.next <= 106, max.whoinc.3.ht,
                                                                  ifelse(delta.agedays.next <= 152, max.whoinc.4.ht,
          ...
          df[!is.na(who.mindiff.next.ht) & (delta.agedays.next < 9*30.4375 | is.na(mindiff.next.ht)),
    ...
    if(!quietly) cat(sprintf("[%s] Exclude single measurements and pairs...\n", Sys.time()))
    ...
    data.df[, exclude := (function(df) {
      ...
      if(length(valid.rows) == 2) {
        ...
        abs.delta.other = with(df, ifelse(!is.na(tbc.other.sd), abs(tbc.sd - tbc.other.sd),
                                          ifelse(!is.na(median.tbc.other.sd), abs(tbc.sd - median.tbc.other.sd), NA)))
    ...
    if(!quietly) cat(sprintf("[%s] Exclude all measurements if maximum threshold of errors is exceeded...\n", Sys.time()))
    # NOTE: restrict analysis to non-missing rows
    data.df[exclude != 'Missing', exclude := (function(subj.df) {
      inc.exc = subj.df[,j=list(exclude.count=sum(!valid(exclude,include.duplicates=T,include.carryforward=T)), include.count=sum(valid(exclude))),keyby=param]
      ...
      # ensure factor levels didn't accidentally get mangled
      return(factor(subj.df$exclude, levels=exclude.levels, ordered=T))
...
sd.median = function(param, sex, agedays, sd.orig) {
  ...
  dt.median = dt[!is.na(sd.orig), list(sex=c(0,1),sd.median=rep(median(sd.orig),2)), by=.(param,ageyears)]
