...
`r which(cleaned_data[, clean_value] == "Exclude-Carried-Forward")[1]`
weight measurement for `subjid` `r cleaned_data[clean_value == "Exclude-Carried-Forward", subjid][1]` as an apparent carried forward value. The measurement remains in the dataset, and it is up to the researcher to determine
...
Users have reported errors running multiple batches with `parallel = TRUE` from
...
```
  ...
  * `quietly = FALSE` enables verbose output, marking the progress of the algorithm
